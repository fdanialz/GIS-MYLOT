...
export default function Dashboard() {
  ...
  const filteredDaerahList = selectedDistrict === 'all' 
    ...
    : SUMMARY_STATS_NS.daerahList.filter(d => d.id === selectedDistrict || d.nama.toLowerCase().includes(selectedDistrict.toLowerCase()));
    ...
    datasets: [
      {
        ...
        data: filteredDaerahList.map(d => d.rmHektar),
      ...
      },
      {
        ...
        data: filteredDaerahList.map(d => d.hskHektar),
      ...
      },
      {
        ...
        data: filteredDaerahList.map(d => d.roaHektar),
  ...
  // District details dictionary with 7 districts
  const districtDetails = {
    seremban: { rm: '4,820', hsk: '12,450', roa: '320', lot: '279,467', rmCount: 48, pmbCount: 16 },
    jempol: { rm: '14,250', hsk: '28,900', roa: '1,450', lot: '65,228', rmCount: 60, pmbCount: 20 },
    pd: { rm: '2,150', hsk: '1,820', roa: '85', lot: '64,565', rmCount: 32, pmbCount: 7 },
    kualapilah: { rm: '8,920', hsk: '15,400', roa: '580', lot: '56,507', rmCount: 30, pmbCount: 18 },
    tampin: { rm: '7,910', hsk: '11,600', roa: '280', lot: '49,413', rmCount: 22, pmbCount: 8 },
    rembau: { rm: '6,480', hsk: '8,200', roa: '410', lot: '39,490', rmCount: 42, pmbCount: 14 },
    jelebu: { rm: '5,120', hsk: '34,500', roa: '920', lot: '32,225', rmCount: 21, pmbCount: 3 }
  };
    ...
    <div className="dashboard-panel">
      ...
      <div className="card-section" style={{ background: 'linear-gradient(135deg, rgba(15, 23, 42, 0.8) 0%, rgba(30, 58, 138, 0.4) 100%)', border: '1px solid rgba(59, 130, 246, 0.3)', marginBottom: '0.75rem' }}>
        <div style={{ fontSize: '0.78rem', fontWeight: 600, color: '#60a5fa', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
          <Eye size={15} /> Real-Time Analytics Portal MRIS (7 Daerah)
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          <div style={{ background: 'rgba(15, 23, 42, 0.6)', padding: '0.6rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: '0.62rem', color: 'var(--text-muted)' }}>Pelawat Unik Portal</div>
          </div>

          <div style={{ background: 'rgba(15, 23, 42, 0.6)', padding: '0.6rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div style={{ fontSize: '0.68rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
              <MousePointerClick size={12} color="#f59e0b" /> Carian & Interaksi Spasial:
            </div>
      ...
      <div className="card-section" style={{ marginBottom: '0.75rem' }}>
        <div className="form-group" style={{ marginBottom: 0 }}>
          <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: '#f59e0b' }}>
            <MapPin size={13} /> Analisis Statistik Mengikut Daerah (7 Daerah):
          </label>
          <select 
          ...
          >
            <option value="all">Semua Daerah (Seluruh Negeri Sembilan)</option>
            ...
            <option value="kualapilah">Daerah Kuala Pilah</option>
            <option value="jelebu">Daerah Jelebu</option>
      ...
      <div className="stat-card-grid">
        <div className="stat-card" style={{ borderLeftColor: '#facc15' }}>
          ...
          <div className="stat-unit">Hektar Warta ({currentDetails.rmCount} Warta)</div>
        </div>
      ...
      <div className="stat-card-grid">
        <div className="stat-card" style={{ borderLeftColor: '#a855f7' }}>
          ...
          <div className="stat-unit">Hektar (Ungu JAKOA)</div>
        </div>

        <div className="stat-card" style={{ borderLeftColor: '#3b82f6' }}>
          <div style={{ fontSize: '0.72rem', color: '#94a3b8', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
            <PieChart size={14} color="#3b82f6" /> LOT KADASTER
          </div>
          <div className="stat-val" style={{ color: '#3b82f6' }}>{currentDetails.lot}</div>
          <div className="stat-unit">Lot NDCDB Terpeta</div>
        </div>
      </div>
      ...
      <div className="card-section">
        <div className="card-title">
          <BarChart3 size={18} color="#3b82f6" /> Perbandingan Keluasan 7 Daerah Negeri Sembilan
        </div>
